...
Output channels (matching train.py's config.NUM_CLASSES=4, one-hot on raw labels):
  0 → Background   (label 0)
  1 → NCR/NET      (label 1)
  2 → Edema        (label 2)
  3 → ET*          (label 3 in one-hot space; BraTS raw label is 4,
                    which falls outside num_classes=4, so this channel
                    received no positive supervision during training)
...
# 3 output channels — TC, WT, ET (confirmed by checkpoint output layer shape [3, 48, 1, 1, 1])
...
def build_val_loader(val_files):
    """
    Uses ConvertToMultiChannelBasedOnBratsClassesd to produce 3-channel
    TC/WT/ET labels, matching the out_channels=3 checkpoint.
    """
...
def load_checkpoint(model, path, device):
    ...
    Handles two checkpoint formats:
      - New style (train.py): torch.save(model.state_dict(), path)
        → the file IS the state_dict (an OrderedDict of tensors)
      - Old style: torch.save({"state_dict": ..., "epoch": ..., ...}, path)
        → the file is a plain dict with a "state_dict" key

    Returns a (epoch, best_acc) tuple for logging; both are None for new-style.
    """
    ...
    if isinstance(raw, dict) and "state_dict" in raw:
        ...
        return raw.get("epoch", None), raw.get("best_acc", None)
    else:
        # New flat format — raw is the state_dict itself
...
def zero_out_channel(batch_image: torch.Tensor, channel: int) -> torch.Tensor:
    """Return a copy of the image with one input modality channel zeroed out."""
...
def run_evaluation(model, loader, device, model_inferer,
                   post_sigmoid, post_pred,
                   acc_func, drop_channel=None):
    """
    Evaluate the model on `loader`.
    If drop_channel is an int, zero out that input modality before inference.
    Labels are already 3-channel TC/WT/ET from ConvertToMultiChannelBasedOnBratsClassesd.
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            run_acc_list.append(acc.cpu().numpy())
...
def build_plot(results: dict, save_path: str):
    """
    results: {scenario_label: np.array of shape (NUM_CLASSES,)}
    """
    ...
    delta_matrix = np.array([results[s] - baseline for s in drop_labels])  # (4, NUM_CLASSES)
    ...
    ax2.set_xticklabels(REGION_NAMES, color=TEXT, fontsize=9)
    ...
    fig.text(
        ...
        ha="center", color=TEXT, fontsize=8, alpha=0.7,
    )
...
def main():
    ...
    model = SwinUNETR(
        in_channels=4,
        out_channels=NUM_CLASSES,   # 4, matching train.py's config.NUM_CLASSES
        feature_size=48,
        drop_rate=0.0,
        attn_drop_rate=0.0,
    ...
    epoch, best_acc = load_checkpoint(model, args.checkpoint, device)
    if epoch is not None:
        print(f"    Loaded checkpoint (epoch {epoch}, best_acc={best_acc:.4f})")
    else:
        print(f"    Loaded checkpoint (flat state_dict — no epoch/acc metadata)")
    ...
    cache_path = args.output_plot.replace(".png", "_cache.json")
    results = {}
    if os.path.exists(cache_path):
        with open(cache_path) as f:
            raw = json.load(f)
        results = {k: np.array(v) for k, v in raw.items()}
        print(f"[*] Resumed {len(results)} scenario(s) from cache: {list(results.keys())}")
    ...
    for label, drop_ch in scenarios:
        ...
        channel_str = "  ".join(
            f"{name}={avg_dice[i]:.4f}" for i, name in enumerate(REGION_NAMES)
        )
        print(f"    {channel_str}  ({elapsed:.1f}s)")

        # Save progress after every completed scenario
        with open(cache_path, "w") as f:
            json.dump({k: v.tolist() for k, v in results.items()}, f, indent=2)
        print(f"    [✓] Progress saved → {cache_path}")

    # ── Summary table ─────────────────────────
    col_w = 8
    header = f"{'Scenario':<22}" + "".join(f"{n:>{col_w}}" for n in REGION_NAMES) + f"{'Mean':>{col_w}}"
    print("\n" + "=" * len(header))
    print(header)
    print("-" * len(header))
    ...
    for label, vals in results.items():
        ...
        if label != "All modalities":
            delta  = mean - np.nanmean(baseline)
            marker = f"  ({delta:+.4f})"
        row = f"{label:<22}" + "".join(f"{v:>{col_w}.4f}" for v in vals) + f"{mean:>{col_w}.4f}{marker}"
        print(row)
    print("=" * len(header))
